...
public class RedisHelper {
	...
	private String host;//="192.168.6.43";
...
//jedis.close();
	 ...
	 * @param ip 访问人的ip地址；
	 */
	public void AddApinum(String api,String ip)
	{
		try {
			if(jedis==null)
			{
			...
			String key = "log" + api;
			if (jedis.hexists(key, ip))//判断是否存在；
			{
				String value = jedis.hget(key, ip);
				Long lv = Long.valueOf(value) + 1;
				jedis.hset(key, ip, lv.toString());
			} else {
				jedis.hset(key, ip, "1");
			}
		}
...
//	}
	...
	public String GetToken(String sktno,String personid) {
		jedis.flushDB();
		String token = java.util.UUID.randomUUID().toString().replaceAll("-", "") + sktno;
		if(jedis.hexists(sktno, "createtime"))
			jedis.hdel(sktno, "createtime");
		//jedis.hset(sktno, "createtime", CommonUtils.getNowDate());
		if(jedis.hexists(sktno, "personid"))
			jedis.hdel(sktno, "personid");
		jedis.hset(sktno, "personid", personid);
		if(jedis.hexists(sktno, "tokenguid"))
			jedis.hdel(sktno, "tokenguid");
		jedis.hset(sktno, "tokenguid", token);
		
		//jedis.set(sktno, token);
		return token;
	}

	// 从redis中取数据写入oracle数据库中；
	public List<String> GetAllRedisKeys() {
		...
		while (it.hasNext()) {
			...
			String retype = jedis.type(key);
			if (retype.equals("string")) {
...
//	// 从hash中获取token表相关数据
//	public TOKEN GetTokenHash(String key) {
//
//		TOKEN token = new TOKEN();
//		try {
//		token.setSktno(key);
//		String perid=jedis.hget(key, "personid");
//		token.setPersonId(Integer.valueOf(perid));
//		String ct=jedis.hget(key, "createtime");
//		token.setCreatetime(CommonUtils.GetDate(ct, ""));
//		token.setTokenguid(jedis.hget(key, "tokenguid"));
...
//			if (retype.equals("string")) {
//				return jedis.get(key);
//			} else if (retype.equals("list")) {
//				List<String> ss = jedis.lrange(key, 0, 100);
//				Gson gson = new Gson();
//
